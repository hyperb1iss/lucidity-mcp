...
def get_git_diff(path: Optional[str] = None) -> Tuple[str, str]:
...
def analyze_code_quality(
    ...
    """
    logger.info(f"Analyzing code quality{f' for {path}' if path else ''}")
    ...
    if not combined_diff:
        return {
            "status": "no_changes",
            "message": "No changes detected in the git diff"
        }
